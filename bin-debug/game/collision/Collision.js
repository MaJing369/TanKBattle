var Collision = (function () {
    ...
    */
    Collision.CheckIntersect = function (obj_1, obj_2, overlap) {
    ...
    /**坦克与地图碰撞 */
    Collision.tankMapCollision = function (tank) {
        var rowIndex = 0;
        var colIndex = 0;
        var rowIndex_1 = 0;
        var colIndex_1 = 0;
        ...
        if (tank.vo.direction == GameData.TANKDIR_UP || tank.vo.direction == GameData.TANKDIR_DOWN) {
            if (tank.vo.moveY <= 0 || tank.vo.moveY + tank.vo.hight >= MapScene.MAPHIGHT)
                return true;
            ...
            if (mapData[rowIndex][colIndex] || mapData[rowIndex_1] && mapData[rowIndex_1][colIndex_1])
                return true;
        }
        else if (tank.vo.direction == GameData.TANKDIR_RIGHT || tank.vo.direction == GameData.TANKDIR_LEFT) {
            if (tank.vo.moveX + tank.vo.width >= MapScene.MAPWIDTH || tank.vo.moveX <= 0)
                return true;
    ...
    Collision.bulletMapCollision = function (bullet) {
        var rowIndex = 0;
        var colIndex = 0;
        ...
        var mapData = MapScene.getInstance().getCurMapData();
        var element = Number(mapData[rowIndex][colIndex]);
